...
##############################################
########## EC2-operator START script #########
##############################################

# location for NON-encrypted & encrypted partition
# /scripts/aws-scripts/ec2-saving-start.sh
...
# Starting EC2 instances with the TAG "EC2-operator=ENABLED" across all AWS regions
LIST_OF_REGIONS=$(aws ec2 describe-regions --output text | cut -f 3)

echo "Starting EC2 instances with the TAG *EC2-operator=ENABLED* across all AWS Regions"
for region in $LIST_OF_REGIONS; do
    ...
    for instance in $LIST_OF_EC2; do
        /usr/local/bin/aws ec2 start-instances --instance-ids "$instance" --region "$region"
    done
...
#/usr/local/bin/aws ec2 associate-address --instance-id i-i-XXXXXX --public-ip XXX.XXX.XXX.XXX --region us-west-1
